...
export type ValueTypes = {
  ...
  ["Mutation"]: AliasType<{
    ...
    updateSources?: [
      { project: string; sources?: ValueTypes["NewSource"][] },
...
Used with paginated listing of projects */
  ...
  ["Query"]: AliasType<{
    findProjects?: [
      { query: string; last?: string; limit?: number },
      ValueTypes["ProjectConnection"]
    ];
    findProjectsByTag?: [
      { tag: string; last?: string; limit?: number },
      ValueTypes["ProjectConnection"]
    ];
    ...
    listProjects?: [
      { last?: string; limit?: number; owned?: boolean },
      ValueTypes["ProjectConnection"]
    ];
  ...
  ["TeamOps"]: AliasType<{
    addMember?: [
      { role: ValueTypes["Role"]; username: string },
      ValueTypes["Member"]
    ];
    createProject?: [{ public?: boolean; name: string }, ValueTypes["Project"]];
    /** Delete team */
...
type MapType<SRC extends Anify<DST>, DST> = DST extends boolean
  ...
  ? {
      ...
      {
        [Key in keyof Omit<DST, "__alias">]: LastMapTypeSRCResolver<
          SRC[Key],
          DST[Key]
        >;
      }
...
const queryConstruct = (t: "Query" | "Mutation" | "Subscription") => (
  o: Record<any, any>
