...
const resolveArg = (f: ParserField) => {
  ...
  const isArrayRequired = !!(
    options && options.find((o) => o === Options.arrayRequired)
  );
  ...
  return `${plusDescription(f.description, "\t")}\t${resolveArgsName(
    f.name
  )}${concatArray(
...
const resolveField = (f: ParserField, enumsAndScalars: string[]) => {
  ...
  const resolvedTypeName =
    f.type.name in typeScriptMap || enumsAndScalars.includes(f.type.name)
      ? "true"
      : resolveValueType(f.type.name);
  if (args && args.length) {
    return `${f.name}?: [{${args
      .map(resolveArg)
      .join(",")}},${resolvedTypeName}]`;
  }
  return `${plusDescription(f.description, "\t")}\t${`${f.name}?` +
    ":"}${resolvedTypeName}`;
};

const AliasType = (code: string) => `AliasType<${code}>`;

const resolveValueTypeFromRoot = (
  i: ParserField,
  rootNodes: ParserField[],
  enumsAndScalars: string[]
) => {
  ...
  if (i.data!.type === TypeDefinition.UnionTypeDefinition) {
    return `${plusDescription(i.description)}["${i.name}"]: ${AliasType(
      `{${i.args
        .map(
          (f) =>
            `\t\t["...on ${f.type.name}"] : ${resolveValueType(f.type.name)}`
        )
        .join(",\n")}\n\t\t__typename?: true\n}`
    )}`;
  }
  ...
  if (i.data!.type === TypeDefinition.InputObjectTypeDefinition) {
    return `${plusDescription(i.description)}["${
      i.name
  ...
  if (i.data!.type === TypeDefinition.InterfaceTypeDefinition) {
    ...
    return `${plusDescription(i.description)}["${i.name}"]:${AliasType(
      `{
\t${i.args
        .map((f) => resolveField(f, enumsAndScalars))
        .join(",\n")};\n\t\t${typesImplementing
